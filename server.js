...
// Database Client
const client = require('./nasa-backend/lib/client');
// Services

// Auth
const ensureAuth = require('./nasa-backend/lib/auth/ensure-auth');
const createAuthRoutes = require('./nasa-backend/lib/auth/create-auth-routes');
...
app.post('/api/me/favorites', async(req, res) => {
    try {
        const newFavorite = await client.query(`
            INSERT INTO favorites (name, url, user_id)
            values ($1, $2, $3)
            RETURNING *
        
        `, [req.body.name, req.body.url, req.userId]);
